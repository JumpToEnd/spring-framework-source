...
public abstract class FactoryBeanRegistrySupport extends DefaultSingletonBeanRegistry {
	...
	/**
	 * 由 FactoryBean 创建的单例对象的缓存:FactoryBean名称到对象
	...
	protected Object getObjectFromFactoryBean(FactoryBean<?> factory, String beanName, boolean shouldPostProcess) {
		/*
		 * 如果 在FactoryBean 中的 FactoryBean#isSingleton() 方法返回 true ， beanName已经在该BeanFactory的单例对象的高速缓存Map集合DefaultListableBeanFactory.singletonObjects中
		 * => 说明当前需要生成的对象是单例的， 可以将生成的对象缓存到 BeanFactory的factoryBeanObjectCache 中，下次如果需要直接从缓存中取就可以了
		 * => 需要说明的是，通过FactoryBean生成的单例对象虽然交由Spring管理，但是是通过 factoryBeanObjectCache 管理的
		 */
		...
		/*
		 * 如果 在FactoryBean 中的 FactoryBean#isSingleton() 方法返回 false
		 * => 说明当前需要生成的对象不是单例的，每次调用新生成一个
		 */
		else {
			// 获取factory管理的对象实例
